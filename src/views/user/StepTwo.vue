...
//                console.log(msg)
                vue.$http.post('http://192.168.1.122:8011/estimation/updateTransaction/' + global_.ID, msg).then(res => {
                  //alert("success999")
                        ...
                        vue.$confirm('此操作将永久删除该节点, 是否继续?', '提示', {
                          ...
                          if(vue.tree.childNodes.length<=1&&node===vue.tree.childNodes[0]){
                            vue.$message({
                              ...
                              type: 'info',
                              message: '当前树已经不能再删除节点，请新建节点后再删除'
                            });
                      ...
                      action: function (vue, node) {
                        ...
                        if(vue.tree.childNodes.length<=1&&node===vue.tree.childNodes[0]){
                          vue.$message({
                            ...
                            type: 'info',
                            message: '当前树已经不能再剪切节点，请新建节点后再删除'
                          });
...
//                            console.log(Msg)
                            ...
                            vue.$http.post('http://192.168.1.122:8011/estimation/TransactionReName/' + global_.ID, Msg).then(res => {
                              //alert("success")
                            }, res => {
                              //alert("fail0")
                            })
       ...
       // Creating the tree
        ...
        var treeMsg;
        this.$http.get('http://192.168.1.122:8011/estimation/getTree/' + global_.ID).then(res => {
