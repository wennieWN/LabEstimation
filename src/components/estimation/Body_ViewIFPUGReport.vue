...
<template>
    ...
    <div class="container">
        ...
        <el-row style="text-align: center">
            <el-button type="primary" v-on:click="prev" >返回</el-button>
        </el-row>
      </div>
    </div>
</template>
...
//                    obj.RET = res.data[i].RET
//                    obj.DET = res.data[i].DET
//                    obj.complexity = res.data[i].complexity
//                    obj.UFP = res.data[i].UFP
//                    data[i] = obj
//                }
...
//    var trans=this.$http.get('http://192.168.1.122:8011/estimation/getRequirement/' + global_.ID).then(response=>{
    ...
    export default {
        ...
        mounted:function () {
            this.$http.get('http://192.168.1.122:8011/estimation/getRequirement/' + global_.ID).then(response=>{
                ...
                console.log(trans.sced);
                this.tType=trans.developmentType;
                this.tPlatform=trans.developmentPlatform;
                this.tLanguage=trans.languageType;
                this.tDB=trans.dbms_Used;
                this.tReliability=trans.rely;
                this.tComplexity=trans.cplx;
                this.tTime=trans.time;
                this.tSchedule=trans.sced;
                this.tProductivity=trans.productivity;
                this.tLabor=trans.cost;
            },response=>{
                alert("error!")
            })
        },
